...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        with dt[0]: 
            # if isinstance(im0s, list) and len(im0s) > 0:
            #     # Crop the left half of the first image in the list
            #     im0s[0] = im0s[0][:, :im0s[0].shape[1] // 2] 
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if flag == 0:
                        ...
                        # print(f"Class: {label}, Confidence: {confidence_str}, Box Width: {box_width}, Box Height: {box_height}")
